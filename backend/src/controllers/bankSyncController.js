...
async function monoRequest(path, options = {}) {
  ...
  if (!response.ok) {
    const error = new Error(
      data?.message || data?.error || "Mono API request failed.",
    );
...
async function initiateBankLink(req, res) {
  try {
    ...
    if (!link) {
      ...
      return res
        .status(502)
        .json({ error: "Mono did not return a bank-link URL." });
    }
...
async function fetchMonoTransactions(monoAccountId) {
  return monoRequest(
    `/accounts/${encodeURIComponent(monoAccountId)}/transactions?paginate=false`,
  );
}
...
async function syncBankAccount(bankAccountId) {
  ...
  if (!bankAccount.mono_account_id) {
    throw Object.assign(
      new Error("This bank account has not finished connecting."),
      { status: 409 },
    );
  }
  ...
  const balance =
    rawBalance == null ? null : moneyFromMinorUnit(rawBalance, currency);
  ...
  let imported = 0;
  if (
    meta.retrieved_data?.includes("transactions") ||
